...
- [ ] H8 Push notifications (owner 2026-07-25). Phase E already has: backend NotificationService + inbox endpoints (register-token, list, read) + FcmDriver (console|mock|firebase) + auto-notify on credit/redemption. PENDING:
  * App: add firebase_messaging + FCM token registration (POST /notifications/register-token), Android 13+ POST_NOTIFICATIONS permission, notification channel + icon, foreground/background handlers, and an in-app INBOX screen (list GET /notifications, mark read, unread badge on home).
  * Backend: enable real FCM (FCM_DRIVER=firebase, reuse auth service account); ensure ALL earn activities notify (game/scratch/spin/streak/referral/offer/ad) + redemption status; ADMIN BROADCAST endpoint — send custom {title, body} to ALL users or a selected user set → create inbox rows + FCM multicast; keep a sent record.
  * Admin: "Send notification" compose screen — title, body, audience (all users / pick specific users), send + sent history.
  * Test real push on the connected device (has Play Services). google-services.json already present.

- [ ] H9 Game reward display uses config, not hardcoded (owner 2026-07-25). game_screen.dart difficulty picker hardcodes _reward{easy:5,medium:10,hard:20}; must read game.coins_per_round config. Fix: add GET /api/game/config -> {easy,medium,hard} coins (from app_config), app picker reads it. (Win popup already uses server rewardPreview correctly.) Build AFTER the running H8 notifications agent finishes (both touch app api_client.dart — avoid conflict). Batch into next app build.

- [ ] H10 Gift store shows ONLY in-stock denominations (owner 2026-07-25). Currently GET /api/gift-cards returns all isActive cards incl. seeded Rs50/100/250 with available=0 -> app shows phantom "out of stock" Rs250. Fix: (1) backend gift-cards.service user-facing list filters to available>0 (pure inventory-driven; drop the sold-out display); (2) remove CATALOG placeholder seeding from seed.ts (catalog auto-creates on inventory upload now); (3) app brand grid must hide brands with no in-stock cards (verify _groupByBrand excludes empty). Admin GET /api/admin/gift-cards can still show all (for management). Backend-only for user list; verify app empty-brand handling. Batch with H8-follow-up build.
