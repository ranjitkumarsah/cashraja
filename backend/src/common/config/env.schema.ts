...
export const envSchema = z
  .object({
    ...
    // PlaytimeAds offerwall (native SDK + S2S postback). APP_KEY inits the SDK
    // AND signs the postback: signature = sha1(user_id + offer_id + amount +
    // APP_KEY + <secret>). SECRET_KEYS is a comma list so more than one secret can
