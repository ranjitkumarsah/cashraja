/**
 * Public legal + informational copy for the Cash Raja marketing site.
 *
 * IMPORTANT: this MUST stay in sync with the in-app copy at
 * `app/lib/features/legal/legal_content.dart`. The public Privacy Policy URL
 * served from `/privacy` is the one referenced by the Google Play listing, so
 * the wording here is a faithful mirror of the Dart source. When the app copy
 * changes, update both places (and the Play Data Safety form).
 *
 * The strings use a lightweight markdown-ish syntax rendered by
 * `PolicyRenderer`: `#`/`##` headings, `- ` bullets, blank-line paragraphs.
 */

export const CONTACT_EMAIL = 'support.cashraja@gmail.com';

/** Effective date; update on each real revision (and re-sync Data Safety). */
export const LAST_UPDATED = 'Effective date: 25 July 2026';
...
## Contact
Questions about your data or this policy? Email ${CONTACT_EMAIL}.`;
...
export const ABOUT_US = `# About Cash Raja
